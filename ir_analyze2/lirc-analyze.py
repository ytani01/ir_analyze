...
class SigData:
    ...
    def load_data(self, mode, infile='', btn='', forever=False, oscillo=False):
        ...
        while True:
            ...
            if not data_start:
                ...
                if mode == 'exec.mode2' and f.cmd[0] == 'ir-ctl':
                    data_start = True
                else:
                    continue
    ...
    # space ...
    #
    def disp_pulse_space(self, button_num):
        if len(self.raw_data) == 0:
            return
        sig_str = self.sig_str.replace(' ', '')
        n = 0
        for i, ch in enumerate(sig_str):
            ...
            ts = self.n_list[i][1] * self.T
            if i < len(sig_str) -1:
                self.print('pulse %d' % tp)
                self.print('space %d' % ts)
            else:
                self.print('pulse %d' % tp)
        return
...
def main(infile, button_name,
    ...
    while True:
        ...
        if sig_data.disp_flag['raw']:
            sig_data.disp_pulse_space(button_num)
            sig_data.print()
